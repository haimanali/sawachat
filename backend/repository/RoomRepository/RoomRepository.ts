...
// this repository handles all the sql queries for chat rooms and their members
export class RoomRepository implements IRoomRepository {
    ...
    // helper to get a room from the database using its primary key
    ...
    // this updates the little preview text (last message) that you see in the chat list
    ...
    public async deleteLastMessagePreview(room_id: number, public_id: string): Promise<IRepositoryLayerResponse> {
        const sql = `
        UPDATE ChatRoom 
        SET last_message_payload = CASE 
            WHEN last_message_payload->>"$.msg_public_id" = ? THEN NULL 
            ELSE last_message_payload 
        END
        WHERE room_id = ?
    `;

        const result = await this.db_conn.executeUpdate(sql, [public_id, room_id]);

        return {
            success: true,
            log_message: "Last message preview updated",
        };
    }

    // this creates a new chat room record
    ...
    public async getRoomsByUserID(user_id: number, cursor: Date | null): Promise<IRepositoryLayerResponse<IRoom[]>> {
        let sql = `select cr.room_id, BIN_TO_UUID(cr.public_id) as public_id, HEX(cr.enc_key) as enc_key, CASE WHEN u.is_ban = true THEN 'User_Banned' ELSE u.nickname END as room_name, u.username as room_subname, cr.type, cr.created_at, other.is_active, cr.last_msg_date, cr.last_message_payload,
            (select m.is_del from Message m where m.room_id = cr.room_id order by m.created_at desc limit 1) as actual_is_del,
